...
def parse_args():
    ...
    # model
    parser.add_argument('--model', type=str, default='llama-3.1-8b', choices=['gpt-4o', 'gpt-4o-mini', 'llama-3-8b', 'llama-3-8b-instruct', 'llama-2-7b', 'llama-2-13b', 'llama-2-70b', 'qwen-2-72b', 'llama-3.1-8b', 'llama-3.1-8b-instruct', 'llama-3.1-70b', 'llama-3.1-405b', 'mistral-0.3-7b'])
    parser.add_argument('--judge_model', type=str, default='gpt-4o', choices=['gpt-4o', 'gpt-4o-mini', 'llama-3-8b', 'llama-3-8b-instruct', 'llama-2-7b', 'llama-2-13b', 'llama-2-70b', 'qwen-2-72b', 'llama-3.1-8b', 'llama-3.1-8b-instruct', 'llama-3.1-70b', 'llama-3.1-405b', 'mistral-0.3-7b'])

    # dataset
    parser.add_argument('--dataset', type=str, default='MATH', choices=['gsm8k', 'MATH', 'AQuA', 'deepmind', 'mmlu', 'numglue', 'sat', 'simuleq', 'SVAMP'])
    ...
    parser.add_argument('--concurrent_size', type=int, default=3)
    parser.add_argument('--max_error_attempt', type=int, default=3, help='maximum number of attempts to find the error')
    ...
    parser.add_argument('--question_token', type=int, default=0)
...
def get_response(messages, model_name, temperature=0):
...
def process_question(example, args, i):
    print('\n' + '*' * 50 + f' Question ID: {i} ' + '*' * 50)
    question_chat_cnt = 0
    question_total_token = 0
    key1 = args.key1
    key2 = args.key2
    ...
    # multiple attempt to obtain a wrong y_before
    error_attempt = 1
    while True:

        # multiple attempt to obtain a normal y_before
        attempt = 1
        while True:
            ...
            y_before, y_before_messages_tokens = get_response(messages, args.model, temperature=0.95)
            question_chat_cnt += 1
            question_total_token += y_before_messages_tokens
            print('y_before messages tokens', y_before_messages_tokens)
            print('y_before response:\n', y_before)
            y_before_tokens, question_total_token = get_tokens(y_before, args.model, question_total_token)
        ...
        y_before_judge_response, y_before_judge_messages_tokens = get_response(messages, args.judge_model, temperature=0)
        question_chat_cnt += 1
        question_total_token += y_before_judge_messages_tokens
        print('y_before judge messages tokens', y_before_judge_messages_tokens)
        print('y_before judge response:\n', y_before_judge_response)
        y_before_judge_tokens, question_total_token = get_tokens(y_before_judge_response, args.judge_model, question_total_token)
        ...
        y_before_extract_response, y_before_extract_messages_tokens = get_response(messages, args.judge_model, temperature=0)
        question_chat_cnt += 1
        question_total_token += y_before_extract_messages_tokens
        ...
        y_before_extract_tokens, question_total_token = get_tokens(y_before_extract_response, args.judge_model, question_total_token)
        ...
        if y_before_score == 0 or error_attempt >= args.max_error_attempt:
            break
        else:
            error_attempt += 1
    ...
    while True:
        try:
            ...
            if args.is_find_error:
                ...
                error, error_messages_tokens = get_response(messages, args.model, temperature=0)
                question_chat_cnt += 1
                question_total_token += error_messages_tokens
                print('find error messages tokens', error_messages_tokens)
                print('find error response:\n', error)
                error_tokens, question_total_token = get_tokens(error, args.model, question_total_token)
                ...
                y_rectify_whole, y_rectify_whole_messages_tokens = get_response(messages, args.model, temperature=0.95)
                question_chat_cnt += 1
                question_total_token += y_rectify_whole_messages_tokens
                print('y_rectify_whole messages tokens', y_rectify_whole_messages_tokens)
                print('y_rectify_whole response:\n', y_rectify_whole)
                y_rectify_whole_tokens, question_total_token = get_tokens(y_rectify_whole, args.model, question_total_token)
        ...
        y_rectify_judge_response, y_rectify_judge_messages_tokens = get_response(messages, args.judge_model, temperature=0)
        question_chat_cnt += 1
        question_total_token += y_rectify_judge_messages_tokens
        ...
        y_rectify_judge_tokens, question_total_token = get_tokens(y_rectify_judge_response, args.judge_model, question_total_token)
        ...
        y_rectify_extract_response, y_rectify_extract_messages_tokens = get_response(messages, args.judge_model, temperature=0)
        question_chat_cnt += 1
        question_total_token += y_rectify_extract_messages_tokens
        ...
        y_rectify_extract_tokens, question_total_token = get_tokens(y_rectify_extract_response, args.judge_model, question_total_token)
    ...
    return {
            ...
            'input': input,
            'answer': answer,
            'question_chat_cnt': question_chat_cnt,
            'question_total_token': question_total_token,
            'attempt': attempt,
...
def main():
    ...
    save_dir = os.path.join(args.save_path, head)
    ...
    try:
        ...
        try:
            ...
            try:
                data_all = load_dataset(data_path, 'main', split="train", cache_dir='./cache')
    ...
    if args.data_num <= len(data_all):
        random.seed(0)
        data_all = random.sample(data_all, args.data_num)
    else:
        repeat_count = args.data_num // len(data_all)
        remainder = args.data_num % len(data_all)
        data_all = data_all * repeat_count + random.sample(data_all, remainder)
    ...
    chat_history.append(
        {
            ...
            'max_error_attempt': args.max_error_attempt,
            'score_threshold': args.score_threshold,
        }
    )

    with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrent_size) as executor:
        futures = [executor.submit(process_question, data_all[i], args, i) for i in range(args.data_num)]
    ...
    total_token = sum([item['question_total_token'] if 'question_total_token' in item else 0 for item in chat_history])
    total_chat_cnt = sum([item['question_chat_cnt'] if 'question_chat_cnt' in item else 0 for item in chat_history])
    ...
    print('Total Statistics:\n',
        ...
        f'Total Chat Cnt: {total_chat_cnt}\n',
