...
def parse_args():
    ...
    parser.add_argument('--given_data', type=int, default=1)
    ...
    parser.add_argument('--judge_model', type=str, default='gpt-4o', choices=['gpt-4o', 'llama-3-8b', 'llama-3-8b-instruct', 'llama-3.1-8b', 'llama-3.1-8b-instruct', 'llama-3.1-70b-instruct-turbo', 'mistral-0.1-7b', 'mistral-0.2-7b-instruct'])
    parser.add_argument('--is_judge_model_api', type=int, default=1)
    parser.add_argument('--is_rectify_model_api', type=int, default=1)
    parser.add_argument('--n_y_before', type=int, default=1)
    parser.add_argument('--n_y_rectify', type=int, default=3)
    parser.add_argument('--n_judge', type=int, default=5)
    parser.add_argument('--y_before_threshold', type=int, default=1)
    parser.add_argument('--y_rectify_threshold', type=int, default=5)
    ...
    parser.add_argument('--frac_len', type=int, default=5)
...
def run_llm(messages, model_name="gpt-4o-2024-05-13", temperature=0.5, top_p=0.8, n=1, max_tokens=512, retry_attempts=3):
...
def main():
    ...
    if args.task == 'gen_y_before' or args.task == 'all':
        ...
        ref_sampling_params = SamplingParams(temperature=0.5, top_p=0.8, max_tokens=512, n=args.n_y_before)
    ...
    if args.task == 'jud_y_before' or args.task == 'all':
        ...
        if args.is_judge_model_api:
            ...
            for message in y_before_judge_messages_format:
                response = run_llm(message, model_name=args.judge_model, temperature=0.5, top_p=0.8, n=args.n_judge, max_tokens=256)
            ...
            for message in y_before_extract_messages:
                response = run_llm(message, model_name=args.judge_model, temperature=0.5, top_p=0.8, n=1, max_tokens=256)
                y_before_extract_responses.append(response)

        else:
            judge_sampling_params = SamplingParams(temperature=0.5, top_p=0.8, max_tokens=256, n=args.n_judge)
            extract_sampling_params = SamplingParams(temperature=0.5, top_p=0.8, max_tokens=256, n=1)
            y_before_judge_responses = [ 
                [response.text for response in responses.outputs]
    ...
    if args.task == 'gen_y_rectify' or args.task == 'all':
        ...
        if args.is_rectify_model_api:
            ...
            for message in y_rectify_messages_format:
                response = run_llm(message, model_name=args.judge_model, temperature=0.5, top_p=0.8, n=args.n_y_rectify, max_tokens=512)
            ...
            ref_sampling_params = SamplingParams(temperature=0.5, top_p=0.8, max_tokens=512, n=args.n_y_rectify)
            y_rectify_responses = [
                [response.text for response in responses.outputs]
