...
if [ "$server_type" == "api" ]; then
    ...
    envsubst < examples/inference/llama3_vllm.yaml > server_logs/${experiment_name}_${suffix}.yaml
    nohup script -f -a -c "API_PORT=8000 llamafactory-cli api server_logs/${experiment_name}_${suffix}.yaml" server_logs/${experiment_name}_${suffix}.log > /dev/null 2>&1 &
fi
...
if [ "$server_type" == "chat" ]; then
    ...
    envsubst < examples/inference/llama3_vllm.yaml > server_logs/${experiment_name}_${suffix}.yaml
    script -f -a -c "FORCE_TORCHRUN=1 NNODES=1 llamafactory-cli chat server_logs/${experiment_name}_${suffix}.yaml" server_logs/${experiment_name}_${suffix}.log
fi